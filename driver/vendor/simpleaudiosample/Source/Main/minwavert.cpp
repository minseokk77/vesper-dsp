...
)
{
    ...
    if (m_DeviceType == eSpeakerDevice || m_DeviceType == eMicArrayDevice1)
    {
        if (_pDataFormat->FormatSize < sizeof(KSDATAFORMAT_WAVEFORMATEX) ||
            !IsEqualGUIDAligned(_pDataFormat->MajorFormat, KSDATAFORMAT_TYPE_AUDIO) ||
            !IsEqualGUIDAligned(_pDataFormat->SubFormat, KSDATAFORMAT_SUBTYPE_PCM) ||
            !IsEqualGUIDAligned(_pDataFormat->Specifier, KSDATAFORMAT_SPECIFIER_WAVEFORMATEX))
        {
            return STATUS_NO_MATCH;
        }

        PWAVEFORMATEX waveFormat = reinterpret_cast<PWAVEFORMATEX>(_pDataFormat + 1);
...
)
{
    ...
    PMODE_AND_DEFAULT_FORMAT modeInfo               = NULL;
    ULONG                   numModes                = 0;
    ULONG                   cbMinSize               = 0;
    NTSTATUS                ntStatus                = STATUS_INVALID_PARAMETER;
    ...
    if (PropertyRequest->Verb & KSPROPERTY_TYPE_GET)
    {
        ...
        if (numModes == 0 || modeInfo == NULL || modeInfo->DefaultFormat == NULL)
        {
            return STATUS_NOT_SUPPORTED;
        }
    ...
    if (PropertyRequest->Verb & KSPROPERTY_TYPE_GET)
    {
        RtlCopyMemory(PropertyRequest->Value, modeInfo->DefaultFormat, cbMinSize);
        PropertyRequest->ValueSize = cbMinSize;
        ntStatus = STATUS_SUCCESS;
    }
